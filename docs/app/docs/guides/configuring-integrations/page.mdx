...
<Table
    headers={['Field', 'Where', 'Purpose']}
    rows={[
        ['extra.scate.apiKey', 'mobile/app.json', 'Scate API key'],
    ]}
/>

Dashboard: <a href="https://scate.io/">scate.io</a>

1. Open the Scate dashboard and create an app.
2. Copy the API key into `extra.scate.apiKey` in `mobile/app.json`.
3. Rebuild the app so the key is embedded via `expo-constants`.
